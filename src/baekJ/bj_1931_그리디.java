...
public class bj_1931_그리디 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < n; i++) {
            ...
            list.add(new Conference(Integer.parseInt(line[0]), Integer.parseInt(line[1])));
        }

        Collections.sort(list, new ConferenceComparator());

        int cnt = 1;
        int end = list.get(0).end;
        for (int i = 1; i < list.size(); i++) {
            Conference now = list.get(i);
    ...
    private static class ConferenceComparator implements Comparator<Conference> {
